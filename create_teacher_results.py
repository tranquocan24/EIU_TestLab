...
export default function ViewResultsPage() {
        ...
        if (scoreRangeFilter && scoreRangeFilter !== 'all') {
            filtered = filtered.filter(r => {
                switch (scoreRangeFilter) {
                    case 'excellent': return r.score >= 90
                    case 'good': return r.score >= 80 && r.score < 90
                    case 'average': return r.score >= 70 && r.score < 80
                    case 'weak': return r.score >= 60 && r.score < 70
                    case 'poor': return r.score < 60
                    default: return true
                }
            })
        }
    ...
    const getScoreBadge = (score: number) => {
        if (score >= 90) return <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-semibold">Excellent</span>
        if (score >= 80) return <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold">Good</span>
        if (score >= 70) return <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-semibold">Fair</span>
        if (score >= 60) return <span className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-semibold">Average</span>
        return <span className="px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs font-semibold">Poor</span>
    }
    ...
    const uniqueClasses = Array.from(new Set(results.map(r => r.className).filter(c => c !== 'N/A')))

    return (
        <div className="space-y-6 animate-fadeInUp">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800">Student Exam Results</h1>
                    <p className="text-gray-600 mt-1">View and analyze student exam results</p>
                </div>
            ...
            <Card>
                <CardContent className="p-6">
                    <div className="space-y-2">
                        <label htmlFor="exam-select" className="text-sm font-medium text-gray-700">Select exam to view results:</label>
                            ...
                            <Card className="border-l-4 border-l-blue-500">
                                <CardContent className="p-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="text-sm text-gray-600 font-medium">Students Participated</div>
                                ...
                                <CardContent className="p-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-2xl font-bold text-gray-800">{stats.average}/100</div>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="p-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-2xl font-bold text-gray-800">{stats.highest}/100</div>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                            ...
                            <Card className="border-l-4 border-l-orange-500">
                                <CardContent className="p-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="text-sm text-gray-600 font-medium">Lowest Score</div>
                                            <div className="text-2xl font-bold text-gray-800">{stats.lowest}/100</div>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                        ...
                                        <tbody>
                                            {filteredResults.map((result, index) => (
                                                <tr key={result.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="py-3 px-4 text-gray-600">{result.className}</td>
                                                    <td className="py-3 px-4">
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-bold text-purple-600">{result.score.toFixed(1)}/100</span>
                                                            {getScoreBadge(result.score)}
                                                        </div>
                                                    </td>
                                                    <td className="py-3 px-4 text-gray-600">{result.timeSpent} min</td>
                        ...
                        <Card>
                            <CardContent className="p-12">
                                <div className="text-center">
                                    ...
                                    <h3 className="text-xl font-semibold text-gray-800 mb-2">No exam results yet</h3>
            ...
            {!selectedExam && (
                <Card>
                    <CardContent className="p-12">
                        <div className="text-center">
                            <div className="text-6xl mb-4">📝</div>
                            <h3 className="text-xl font-semibold text-gray-800 mb-2">Select an exam to view results</h3>
                            <p className="text-gray-500">Please select an exam from the list above to view detailed results</p>
                        </div>
                    </CardContent>
                </Card>
            )}
