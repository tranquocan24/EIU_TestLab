...
import { CheckCircle, XCircle, Clock, ArrowLeft, Award, FileText } from 'lucide-react'
    ...
    const loadAttemptDetail = async () => {
        try {
            ...
            const transformedAttempt: AttemptDetail = {
                ...
                answers: data.answers.map((answer: { id: string; questionId: string; selectedOption: string | null; isCorrect: boolean; points: number; question: { id: string; questionText?: string; question?: string; points: number; order: number; options: { id: string; text?: string; option?: string; isCorrect: boolean }[] } }) => ({
                    id: answer.id,
                    ...
                    question: {
                        ...
                        questionText: answer.question.questionText || answer.question.question || '',
                        options: answer.question.options.map((opt: { id: string; text?: string; option?: string; isCorrect: boolean }) => ({
                            id: opt.id,
                ...
                <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <div className="text-center p-4 bg-white rounded-lg shadow-sm">
                            <Award className={`h-8 w-8 mx-auto mb-2 ${getScoreColor(attempt.score)}`} />
                        ...
                        <div className="text-center p-4 bg-white rounded-lg shadow-sm">
                            ...
                            <div className="text-sm text-gray-600 mt-1">Thời gian làm bài</div>
                        </div>
                    ...
                    <div className="mt-6 pt-6 border-t grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <span className="ml-2 font-medium">{formatDate(attempt.startedAt)}</span>
                        </div>
                        <div>
                            <span className="text-gray-600">Hoàn thành:</span>
                            <span className="ml-2 font-medium">{formatDate(attempt.submittedAt)}</span>
                        </div>
                    </div>
                </CardContent>
            </Card>
                ...
                {[...attempt.answers]
                    ...
                    .map((answer, index) => {
                        return (
                            <Card
                                key={answer.id}
                                className={`border-2 ${answer.isCorrect
                                ...
                                <CardHeader className="pb-3">
                                    <div className="flex items-start justify-between gap-4">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <div className={`text-2xl font-bold ${answer.isCorrect ? 'text-green-600' : 'text-red-600'
                                                }`}>
                                ...
                                <CardContent className="space-y-3">
                                    {answer.question.options.map((option) => {
                                        ...
                                        if (isCorrect) {
                                            ...
                                            icon = <div className="h-5 w-5 rounded-full border-2 border-gray-300 flex-shrink-0" />
                                        }
